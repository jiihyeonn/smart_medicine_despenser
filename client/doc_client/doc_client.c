...
void* recv_msg(void* arg)
{
    ...
    while (1) {
                 ...
                 && g_step == STEP_PK_WAIT_MEDINFO)
        {
          ...
          /* 6.2 추가 */
          /* ① 요청자(ARD)에게 "배출 시작" 통지 → LCD가 Dispensing... 으로 전환,
           *    ARD가 ST_WAIT → ST_WAIT_DISPENSE 로 넘어가서 짧은 타임아웃을 벗어남 */
          snprintf(send_buf, sizeof(send_buf), "[%s]PICKUP@%s@%d\n",
                   pending_requester, pending_med, pending_count);
          write(*sock, send_buf, strlen(send_buf));
          usleep(200000);   /* ESP 수신 안정화 간격 */

          /* ② STM32에 배출 승인 (DB 변경 없음, ServoChannel 포함) */
          g_step = STEP_PK_WAIT_DONE;
          snprintf(send_buf, sizeof(send_buf), "[STM]PICKUP@%s@%d@%d\n",
          pending_med, pending_count, pending_servo);
          write(*sock, send_buf, strlen(send_buf));
        }
                 ...
                 g_step == STEP_PK_WAIT_DONE)
        {
            ...
            /* 추가 06.02 */
            // 1. 노크 패킷 전송
            snprintf(send_buf, sizeof(send_buf), "[%s]WAKEUP\n", pending_requester);
            write(*sock, send_buf, strlen(send_buf));
            ...
            // 3. 진짜 실패 메시지 전송
            snprintf(send_buf, sizeof(send_buf), "[%s]PICKUP_FAIL@FAIL\n",            pending_requester);
            write(*sock, send_buf, strlen(send_buf));
       ...
       //     pending_count = decided;
        ...
        else if (!strcmp(pArray[0], "SQL") && !strcmp(pArray[1], "UPDATEDB"))
        {
            ...
            /* (픽업) Stock 감소 완료 → 배출 완전 종료 */
              ...
              // 1. 와이파이 모듈을 깨우기 위한 노크(더미) 패킷 전송!
                snprintf(send_buf, sizeof(send_buf), "[%s]WAKEUP\n", pending_requester);
                write(*sock, send_buf, strlen(send_buf));
                
                // 2. 모듈이 정신 차릴 수 있게 0.3초 대기
                usleep(300000); 
                
                // 3. 진짜 배출 완료 메시지 전송
                snprintf(send_buf, sizeof(send_buf), "[%s]PICKUP_DONE@%s@%d\n",
                         pending_requester, pending_med, pending_count);
                write(*sock, send_buf, strlen(send_buf));
